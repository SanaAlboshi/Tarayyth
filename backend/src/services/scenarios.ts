...
import { AnalysisInput, AnalysisResult, runAnalysis } from "./analysis";
...
const SCENARIO_META: Record<ScenarioKey, { title: string; description: string }> = {
  salary_drop_20: {
...
function applyScenario(input: AnalysisInput, key: ScenarioKey): AnalysisInput {
...
function timeline(oldScore: number, newScore: number): { month: number; score: number }[] {
  const points: { month: number; score: number }[] = [];
  for (let i = 0; i <= 6; i++) {
    const t = i / 6;
...
export async function runScenario(req: ScenarioRequest): Promise<ScenarioOutcome> {
  const meta = SCENARIO_META[req.scenario];
  const modifiedInput = applyScenario(req.input, req.scenario);
  const newAnalysis = await runAnalysis(modifiedInput);
...
{"aiExplanation": "شرح احترافي (سطران)", "advice": "نصيحة عملية قابلة للتنفيذ"}`;
    const ai = await generateJson<{ aiExplanation: string; advice: string }>(prompt);
    if (ai.aiExplanation) aiExplanation = ai.aiExplanation;
    if (ai.advice) advice = ai.advice;
  } catch (err) {
